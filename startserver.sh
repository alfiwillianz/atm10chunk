...
if [ ! -e server.properties ]; then
    printf "allow-flight=true\nmotd=All the Mods 10 To the Sky\nmax-tick-time=-1\nlevel-type=minecraft\:normal" > server.properties
fi

if [ -d mods.override ]; then
    echo "Copying mods.override to mods..."
    cp -r mods.override/* mods/
fi

if [ -d config.override ]; then
    echo "Copying config.override to config..."
    cp -r config.override/* config/
fi
